...
********************************************************************/
#ifndef TASMOTA       // only when compiling stand-alone
...
#if defined(__EMSCRIPTEN__)
/* For WASM builds, omit build date/time to avoid unnecessary changes in berry.js */
#define repl_prelude_prefix                                         \
    FULL_VERSION "\n"                                               \
    "[" COMPILER "] on " OS_NAME " (default)"
#else
/* For native builds, include build date/time */
#define repl_prelude_prefix                                         \
...
#define repl_prelude_color                                          \
    "\033[32m" repl_prelude_prefix "\033[39m\n\r"
...
static int load_script(bvm *vm, int argc, char *argv[], int args, const char * script)
{
    int res = 0;
#if defined(__EMSCRIPTEN__)
    /* In browser/WASM mode, never enter REPL automatically.
     * The JavaScript API (berry_execute, etc.) is used instead.
     * REPL mode would cause an infinite loop waiting for input. */
    int repl_mode = 0;
    be_writestring(repl_prelude_color);
    be_writestring("REPL disabled in browser mode. Use JavaScript API.\n");
#else
...
/* External function to set the global VM pointer for JavaScript-to-Berry API */
#if defined(__EMSCRIPTEN__)
extern void berry_set_vm(bvm *vm);
#endif
...
#if defined(__EMSCRIPTEN__)
    /* Set the global VM pointer for JavaScript-to-Berry execution API */
    berry_set_vm(vm);
#endif
...
#if defined(__EMSCRIPTEN__)
    /* In browser/WASM mode, do NOT delete the VM.
     * The VM must remain alive for JavaScript to call berry_execute() etc.
     * Memory will be freed when the page is closed. */
